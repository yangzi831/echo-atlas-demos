 ...
 server.on('upgrade',(req,socket,head)=>{
  ...
  wss.handleUpgrade(req,socket,head,client=>{
   ...
   upstream.on('message',raw=>{
    ...
    if(event==='task-failed')fail(m.header?.error_code==='Workspace.AccessDenied'?'当前密钥无法访问指定工作空间，请清空可选 Workspace ID 或填写有权限的工作空间；原录音仍会保存。':'转写服务拒绝任务，请检查 DashScope 密钥、地域及 Fun-ASR 权限；原录音仍会保存。');
   });
