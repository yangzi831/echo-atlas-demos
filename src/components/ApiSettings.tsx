...
import {hasApiSettings,readApiSettings,saveApiSettings,clearApiSettings,defaultApiSettings} from '../services/apiSettings';
import '../styles/api-settings.css';
export function ApiSettings() {
 const dialog=useRef<HTMLDialogElement>(null);
 const [settings,setSettings]=useState(defaultApiSettings);
 const [message,setMessage]=useState('');
 const [personal,setPersonal]=useState(hasApiSettings());
 ...
 return <>
  <button type="button" className="api-settings-trigger" title="API 设置" aria-label="打开 API 设置" onClick={()=>{setSettings(readApiSettings());setPersonal(hasApiSettings());setMessage('');dialog.current?.showModal();}}>⚙</button>
  <dialog ref={dialog} className="api-settings-dialog" onClick={event=>{if(event.target===dialog.current)dialog.current.close();}}>
   <form onSubmit={event=>{event.preventDefault();try{saveApiSettings(Object.fromEntries(Object.entries(settings).map(([k,v])=>[k,v.trim()])) as typeof settings);setPersonal(true);setMessage('已保存在此浏览器。新的分析请求立即使用，转写配置在下次开始聆听时生效。');}catch{setMessage('浏览器禁止本地存储，未能保存设置。');}}}>
    ...
    <p>当前配置来源：{personal?'浏览器个人配置（优先，不混用服务端密钥）':'服务端默认配置'}。保存后，未填写的密钥将提示缺少配置；清除本地配置才恢复服务端默认值。</p>
    ...
    <div className="api-settings-actions"><button type="button" onClick={()=>{try{clearApiSettings();setPersonal(false);setSettings({...defaultApiSettings});setMessage('已清除本地配置，将使用服务端默认配置。');}catch{setMessage('清除失败，请检查浏览器存储权限。');}}}>清除本地配置</button><button type="submit">保存配置</button></div>
   </form>
  </dialog>
 </>;
}
