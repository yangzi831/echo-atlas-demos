...
export function aiSettings() {const {apiKey,model}=readApiSettings();return {...(!import.meta.env.DEV || hasApiSettings() ? {source:'browser'} : {}),apiKey,model};}
export function asrSettings() {const {asrKey,region,workspace}=readApiSettings();return {...(!import.meta.env.DEV || hasApiSettings() ? {source:'browser'} : {}),asrKey,region,workspace};}
