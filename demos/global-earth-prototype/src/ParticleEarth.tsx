...
export function ParticleEarth({ cities, selectedCity, onSelectCity, onEnterCity }: Props) {
  ...
  useEffect(() => {
    ...
    cities.forEach((city, i) => { const root = new THREE.Group(), position = vector(city.lat, city.lng, RADIUS * 1.035), color = city.hasPublicMemories ? palette[i % palette.length] : 0x649ba3, scale = city.hasPublicMemories ? .98 + Math.min(city.echoes, 24) * .025 : .58 + random(i, 71) * .18; root.position.copy(position); root.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), position.clone().normalize()); root.scale.setScalar(scale);
      ...
      const label = document.createElement('div'); label.className = `earth-city-label${city.hasPublicMemories ? ' has-public-memories' : ''}`; label.textContent = city.name; label.hidden = true; label.setAttribute('aria-hidden', 'true'); host.appendChild(label); markers.push({ city, root, hit, core, ring, glow, label, scale }); });
