...
const server=await createServer({server:{middlewareMode:true,ws:false},appType:'custom'});test.after(()=>server.close());
...
test('批次串行处理，不丢掉积压的较早句子',async()=>{
 ...
 const listener=new SemanticListener(pact,async()=>{},()=>{},async()=>{});
...
test('暂不完整的对话在停止时再次分析，并抑制重复片段',async()=>{
 ...
 const listener=new SemanticListener(pact,async()=>{count++;},()=>{},async()=>{});
